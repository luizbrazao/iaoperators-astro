// Datos del silo 2 — Integración y automatización de sistemas (core evergreen).
// Solo ES por ahora: la keyword primaria del hub es "integración de sistemas"
// (ver src/data/keywords-primarias.ts y docs/plan-arquitectura-2026-08.md §6.1).
//
// REGLA ANTI-DOORWAY (§7.4 del plan): la mayor parte del contenido de cada caso
// es propio —síntoma, por qué no se compra la solución, patrones concretos,
// entregables y FAQ—. Lo compartido entre casos se resume y se enlaza al hub en
// lugar de repetirse. Si algún día dos casos empiezan a parecerse, el problema
// no es el template: es que uno de los dos no merece página.
//
...
export interface CasoIntegracion {
  ...
  schemaName?: string;
  /** `serviceType` del Service en el JSON-LD. Sin él, el genérico del template. */
  serviceType?: string;
  /** CTA discreto bajo el párrafo del hero. Sin él no se renderiza nada. */
  heroCta?: { label: string; href: string };
  /**
   * Overrides de los labels del template ("Relacionado con este caso",
   * "Preguntas de este caso", "Otros casos"). Sin ellos se mantienen los
   * textos originales, así el cambio de un caso no arrastra a sus hermanos.
   */
  labels?: { relacionado?: string; faqTitle?: string; otros?: string };
  intro: string;
...
/** Lo común a todos los casos: se resume aquí y se enlaza al hub, no se repite. */
export const SHARED = {
  proceso: {
...
export const CASOS: Record<CasoSlug, CasoIntegracion> = {
  erp: {
    ...
    labels: {
      relacionado: "Relacionado con la integración de ERP",
